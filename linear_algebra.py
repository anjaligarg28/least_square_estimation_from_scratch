...
class LinearAlgebra:
    ...
    def add_column(self, matrix, column, index):
        new_matrix = []
        row, col = self.get_row_col(matrix)
        for i in range(row):
            new_row = matrix[i]
            # new_row.append(0)
            if(index < col):
                new_row[index+1:] = matrix[i][index:]
                new_row[index] = column[i]
            else:
                new_row.append(column[i])
            new_matrix.append(new_row)
        return new_matrix

    def inverse_matrix_gaussian_elimination(self, matrix):
        n = len(matrix)
        identity = [[1 if i == j else 0 for j in range(n)] for i in range(n)] # Create an identity matrix of the same size as the input matrix

        for i in range(n): # Append the identity matrix to the right of the original matrix
            matrix[i] += identity[i]

        # Apply Gauss-Jordan elimination to transform the left half into an identity matrix
        for i in range(n):
            if matrix[i][i] == 0: # If the diagonal element is 0, find a non-zero element in the same column
                for j in range(i+1, n):
                    if matrix[j][i] != 0:
                        matrix[i], matrix[j] = matrix[j], matrix[i] # Swap rows
                        break
                else: # If no non-zero element is found, the matrix is singular (non-invertible)
                    raise ValueError("Matrix is singular and cannot be inverted")

            factor = matrix[i][i]
            for j in range(2*n): # Scale the row to make the diagonal element 1
                matrix[i][j] /= factor

            for j in range(n): # Make all other elements in the current column 0
                if i != j:
                    factor = matrix[j][i]
                    for k in range(2*n):
                        matrix[j][k] -= factor * matrix[i][k]

        inverse = [row[n:] for row in matrix] # Extract the right half of the matrix as the inverse
        return inverse

    def matrix_addition(self, matrix1, matrix2):
        row1, col1 = self.get_row_col(matrix1)
        row2, col2 = self.get_row_col(matrix2)
        new_matrix = []
